...
class BoardAPITestCase(APITestCase):   
    ...
    def setUp(self):
        ...
        self.common_user = User.objects.create_user(email=fake.email(),password=fake.password())
...
class CommunityAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.admin_token}')
        self.board = Board.objects.create(name=fake.name(),context=fake.sentence())
        self.post = Post.objects.create(user=self.admin_user,board=self.board,title=fake.sentence(),content=fake.sentence())
        self.comment = Comment.objects.create(user=self.admin_user,post=self.post,content=fake.sentence())
    ...
    def test_create_post(self):
        data = {"board":self.board.id,"title":fake.sentence(),"content":fake.sentence()}
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.admin_token}')
    ...
    def test_update_post(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.admin_token}')
        response = self.client.patch(reverse('post-detail', kwargs={'pk': self.post.id}),data=data)
        self.assertEqual(response.status_code,200)
    ...
    def test_update_comment(self):
        data = {"content":fake.sentence()}
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.admin_token}')
        response = self.client.patch(reverse('comment-detail', kwargs={'pk': self.comment.id}),data=data)
        self.assertEqual(response.status_code,200)
